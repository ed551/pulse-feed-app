...
- **Primary Model:** Use **Gemini 1.5 Flash** (`gemini-1.5-flash`) for all standard AI interactions.
- **Fallback Logic:** If `gemini-1.5-flash` is unavailable, fall back to a sequence of models including **Gemini 1.5 Flash-8B**, **Gemini 2.0 Flash**, and **Gemini 1.5 Pro**. This logic includes mandatory delays on quota/billing errors to prevent rate-limit loops. Logic is centralized in `src/lib/ai.ts` and `server.ts`.
